...
task main()
{
   ...
   while(getButtonPress(1) == 1) {}; eraseDisplay();
    ...
    for(int i=0;i<5;i++)
       for(int j=0;j<5;j++)
       {
          if(i==0 && j==0)dt[i][j]= SS[i][j];
          else if(i==0) dt[i][j]=dt[i][j-1]+SS[i][j];
          else if(j==0) dt[i][j]=dt[i-1][j]+SS[i][j];
          else dt[i][j]=max(dt[i-1][j], dt[i][j-1])+SS[i][j];
       }
    ...
    while(r!=0 || c!=0)
    {
      ...
      stopMotor();
    displayBigTextLine(5,"tot score: %d",result); //this is display final score
